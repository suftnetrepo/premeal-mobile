...
import { StyledPage, StyledPressable, Stack, theme } from "fluent-styles";
  ...
  if (!active) {
    return (
      <StyledPage flex={1} backgroundColor="#F8F8F8" alignItems="center" justifyContent="center" padding={24}>
        <Text fontSize={15} color={COLORS.textMuted} textAlign="center" marginBottom={16}>
        ...
        <StyledPressable onPress={() => router.replace("/")} backgroundColor={COLORS.primary} borderRadius={999} paddingHorizontal={20} paddingVertical={12}>
          <Text variant="button" color="#FFFFFF">Set address</Text>
        </StyledPressable>
      </StyledPage>
    );
  }
    ...
    <StyledPage showStatusBar backgroundColor={theme.colors.gray[1]}>
      <StyledPage.Header.Full>
        <Stack vertical flex={1} paddingHorizontal={H_PAD} paddingTop={6} paddingBottom={14} gap={12}>
          <StyledPressable onPress={() => setPickerOpen(true)} flexDirection="row" alignItems="center" gap={6}>
              ...
              <Text fontSize={13} fontWeight="700" color={COLORS.textPrimary} numberOfLines={1} flex={1}>
                {active.formattedAddress}
              </Text>
              <Text fontSize={12} fontWeight="700" color={COLORS.primary}>Change</Text>
            </StyledPressable>
            ...
            <Stack flexDirection="row" alignItems="center" gap={10}
              ...
              <Text fontSize={13} color={COLORS.textMuted}>Search by restaurant or entrée</Text>
            </Stack>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
              <StyledPressable
                ...
                <Text fontSize={13} fontWeight="700" color={cuisineFilter === null ? "#FFFFFF" : COLORS.textSecondary}>
                  All
                </Text>
              </StyledPressable>
              {SUPPORTED_CUISINES.map((c) => (
                <StyledPressable
                  ...
                  <Text fontSize={13} fontWeight="700" color={cuisineFilter === c ? "#FFFFFF" : COLORS.textSecondary}>
                    {c}
                  </Text>
                </StyledPressable>
              ))}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: H_PAD, paddingBottom: 24 }}>
        <Stack horizontal alignItems="center" justifyContent="space-between" marginBottom={14}>
          <Text fontSize={19} fontWeight="800" color={COLORS.textPrimary}>Near you</Text>
          {!isLoading && !error && <Text fontSize={13} color={COLORS.textMuted}>{filtered.length} places</Text>}
        </Stack>
        ...
        {error && (
          <Stack padding={14} borderRadius={12} backgroundColor={COLORS.errorLight}>
            <Text variant="body" color={COLORS.error}>Could not load restaurants. Check your connection.</Text>
          </Stack>
        )}
        ...
        {!isLoading && !error && filtered.length === 0 && (restaurants ?? []).length > 0 && (
          <Stack alignItems="center" paddingVertical={32} gap={8}>
            ...
            <StyledPressable onPress={() => setCuisineFilter(null)}>
              <Text fontSize={13} fontWeight="700" color={COLORS.primary}>Browse all cuisines</Text>
            </StyledPressable>
          </Stack>
        )}
        ...
        {!isLoading && !error && (restaurants ?? []).length === 0 && (
          <Stack alignItems="center" paddingVertical={32} gap={8}>
            ...
            <StyledPressable onPress={() => setPickerOpen(true)} marginTop={4}>
              <Text fontSize={13} fontWeight="700" color={COLORS.primary}>Try a different address →</Text>
            </StyledPressable>
          </Stack>
        )}
