  ...
  return (
    <StyledPage flex={1} backgroundColor={COLORS.bg} showStatusBar statusBarProps={{ barStyle: "dark-content" }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        {/* No stock-photo fallback here on purpose — see the same fix
            already applied on the home screen's restaurant cards. A
            random Unsplash/Picsum image with no thematic control was
            tried and reverted more than once in this project; an honest
            "no real photo yet" placeholder is preferable to pretending
            one exists. */}
        {restaurant.imageUrl ? (
          <StyledImageBackground source={{ uri: restaurant.imageUrl }} height={170} />
        ) : (
          <Stack height={170} backgroundColor={COLORS.primaryLight} alignItems="center" justifyContent="center">
            <StyledText fontSize={48}>{cuisineEmoji(restaurant.cuisine)}</StyledText>
          </Stack>
        )}
